...
func Handle(w http.ResponseWriter, r *http.Request) {
	...
	if err := createBucket(minioInstance, name); err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		errMsg := fmt.Sprintf("Cannot create MinIO bucket: %s", err)
		log.Println(errMsg)
		w.Write([]byte(errMsg))
		return
	}

	subject := "nats-create-minio-policy"
	if val, ok := os.LookupEnv("topic_create_minio_policy"); ok {
		subject = val
	}
